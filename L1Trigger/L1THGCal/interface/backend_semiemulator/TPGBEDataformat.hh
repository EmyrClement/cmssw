...
namespace TPGBEDataformat{
  ...
  public:
    ...
    void accumulate(const TPGTCFloats &t){
      accumulate(t.getEnergy(), t.getROverZ(), t.getPhi(), t.getZ(), t.getLayer(), t.getCMSSWIndex());
    }    
    ...
    void accumulate(uint32_t e, uint16_t roz, uint16_t phi, uint16_t z, uint16_t l, int16_t cmsswIndex){
      bool cee(l<=26);
      ...
      cmsswIndices_.push_back(cmsswIndex);
    }
    ...
    bool isLocalMaximum(bool debug=false) const {
      ...
      if (e == 0) return false;
      if (debug) {
        std::cout << "TcAccumulatorFW::isLocalMaximum, totE = " << e << std::endl;
      }
      if (vNN.size() == 1) {
        return true; // If only one neighbour, it is always a local maximum
      }
      for (unsigned i(0); i < vNN.size(); i++){
      if (debug) {
        std::cout << "Checking neighbour " << i << " with totE = " << vNN[i]->totE() << std::endl;
      }
    ...
    void setCmsswIndices(const std::vector<int>& indices) { cmsswIndices_ = indices; }
    const std::vector<int>& getCmsswIndices() const { return cmsswIndices_; }
